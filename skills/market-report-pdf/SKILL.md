...
description: Use when Codex needs to generate a polished PDF marketing report with charts, scores, findings, competitor data, and action plans using the bundled PDF report script.
---
...
**Generate the report:**
```bash
uv run --with reportlab python scripts/generate_pdf_report.py /tmp/report_data.json "MARKETING-REPORT-<domain>.pdf"
```
...
```bash
uv run --with reportlab python scripts/generate_pdf_report.py
...
- Footer: "Generated by AI Marketing Suite for Codex"
...
| Issue | Solution |
|---|---|
| `ModuleNotFoundError: No module named 'reportlab'` | Run with `uv run --with reportlab ...` |
